	...
	for (int iter = 0; iter < T; iter++) {
		...
		#pragma omp parallel num_threads(num_threads) firstprivate(num_bodies, t)
		{
			...
			for (int i = 0; i < num_bodies; i++) {
				...
				bodySet[i].velocity.x += F.x / bodySet[i].mass * t;
				bodySet[i].velocity.y += F.y / bodySet[i].mass * t;
			}

			#pragma omp for
			for (int i = 0; i < num_bodies; i++) {
				bodySet[i].position.x += bodySet[i].velocity.x * t;
				bodySet[i].position.y += bodySet[i].velocity.y * t;
			}
		}
